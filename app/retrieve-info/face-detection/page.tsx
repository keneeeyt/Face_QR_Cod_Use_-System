...
const FaceDetectionPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post("/api/face-scan", {
        face_data: JSON.stringify(Array.from(descriptor)),
      });

      const data = res.data;
      if (res.status === 200) {
        setShowMessage("User found!");
        setIsError(false);
        setUser(data.user);
        setShowCard(true);
      } else {
    ...
    <main className="max-w-full border mx-auto p-20 h-screen">
      <div className="flex flex-col items-center justify-center gap-10">
        ...
        <div className="flex flex-col justify-start mt-4 gap-5 w-full max-w-xl">
          ...
          <p
            onClick={() => router.push("/")}
            className="text-blue-500 cursor-pointer mt-5 text-center"
          >
            back to home page
          </p>
        </div>
      ...
      <div className="col-span-2">
        {showMessage && (
          <SuccessMessage message={showMessage} isError={isError} />
        )}
        {showCard && user && (
          <IdentificationCard
            user={user}
